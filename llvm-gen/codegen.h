...
namespace cg_llvm {
    ...
    struct scope_data {
        ...
        llvm::Value* add_variable(std::string_view name, llvm::Type* type) const;
    ...
    llvm::Value* generate_method_call(const ast::ast_node& node, scope_data& data);
    llvm::Value* generate_expression(const ast::ast_node& node, scope_data& data);
    ...
    llvm::Value *generate_initialization(const ast::ast_node &node, scope_data &data);
